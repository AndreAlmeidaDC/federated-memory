# AGENTS.md — Adaptador para Kimi Code CLI (MoonshotAI)

Este arquivo segue a convenção `AGENTS.md` usada pelo Kimi Code CLI v1.44.
O arquivo pode ser gerado via `/init` no CLI ou criado manualmente — o conteúdo abaixo é o que o Kimi consome.
Traduz o contrato neutro de [`../../00-global/AGENT.md`](../../00-global/AGENT.md) para o consumo do Kimi.
...
- A regra vale em todos os modos do Kimi Code CLI (interativo, plan, auto). O nível de aprovação altera a UX, não a política.
...
O vault é a memória persistente. O contexto da sessão Kimi é volátil.
